...
def beagle3_htParslConfig(params):
    ...
    scheduler_options = [f"#SBATCH --gres=gpu:{params['max_workers_per_node']}", 
            "#SBATCH --partition=beagle3",
            f"#SBATCH --time={params['walltime']}",
    ]
    ...
    config = ParslConfig(
        executors=[
            HighThroughputExecutor(
                label="htex_slurm",
                available_accelerators=params['max_workers_per_node'],  # Pin each worker to a different GPU
                max_workers_per_node=params['max_workers_per_node'],
                provider=SlurmProvider(
                    ...
                    nodes_per_block=params['nodes_per_block'], 
                    min_blocks=params['min_num_blocks'],
                    max_blocks=params['max_num_blocks'],  
                ),
            )
...
def beagle3_localParslConfig(params):
    ...
    local_htex = Config(
        executors=[
            HighThroughputExecutor(
                label="htex_slurm",
                max_workers_per_node=params['max_workers_per_node'], # vs max_workers
                available_accelerators=4,
                mem_per_worker = 8,
                worker_debug=True,
...
#             HighThroughputExecutor(
#                 label="htex_slurm",
#                 max_workers_per_node=1, # vs max_workers
#                 available_accelerators=4,
#                 mem_per_worker = 8,
#                 worker_debug=True,
#                 cores_per_worker=8, # how many cores per worker #nodes_per_block, 2 is usually enough or 1.
