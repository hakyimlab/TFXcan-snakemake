...
data.table::fwrite(forDB_geno_file, file=glue("{opt$output_folder}/{opt$pop}.geno.db_compatible.txt"), sep='\t', quote=F, row.names=F, col.names = F)
data.table::fwrite(forDB_geno_file, file=glue("{opt$output_folder}/{opt$pop}.geno.db_compatible.txt.gz"), sep='\t', quote=F, row.names=F, compress='gzip', col.names = F)
