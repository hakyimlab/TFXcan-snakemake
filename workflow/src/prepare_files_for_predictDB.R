...
option_list <- list(
    make_option("--individuals", help='a comma separated list of individuals'),
    make_option("--input_file_pattern", help='a pattern for the predicted enpact scores'),
    make_option("--output_directory"),
    make_option("--include_models", default = NULL),
    make_option("--filtered_GWAS_file"),
    make_option("--phenotype")
)
...
# opt <- list()
# opt$individuals <- "HG00100,HG00114,HG00099,HG00109,HG00111,HG00104,HG00105,HG00096,HG00103,HG00101,HG00102,HG00108,HG00097,HG00112,HG00106,HG00110"
# opt$input_file_pattern <- "/project2/haky/temi/projects/TFXcan-snakemake/data/enpact_predictions/asthma_children/{}.asthma_children.aggByCollect.2024-01-31.csv.gz"
# opt$include_models <- NULL
# opt$filtered_GWAS_file <- '/project2/haky/temi/projects/TFXcan-snakemake/data/collection/asthma_children/asthma_children.filteredGWAS.txt.gz'
...
data.table::fwrite(annot_dt, file = file.path(opt$output_dir, paste(opt$phenotype, 'tf_tissue_annot.txt', sep='.')), quote=F, row.names = T, col.names = T, sep = '\t')
data.table::fwrite(enpact_scores_dt, file = file.path(opt$output_dir, paste(opt$phenotype, 'enpact_scores.txt', sep='.')), quote=F, row.names = F, col.names = T, sep = '\t')
