...
height_ss <- data.table::fread('/beagle3/haky/users/temi/projects/TFXcan-snakemake/data/sumstats/standing_height.gwas_sumstats.tsv.gz') %>%
    dplyr::rename(chrom = chromosome, pos = base_pair_location, ref = other_allele, alt = effect_allele, beta = beta, se = standard_error, pval = neg_log_10_p_value) %>% 
    dplyr::filter(nchar(alt) == 1 & nchar(ref) == 1) %>%
    dplyr::mutate(zscore = beta/se, variant_id = paste(chrom, pos, ref, alt, sep = '_'), pval = 10^-(pval)) %>%
    dplyr::filter(pval <= 1e-6)
```
